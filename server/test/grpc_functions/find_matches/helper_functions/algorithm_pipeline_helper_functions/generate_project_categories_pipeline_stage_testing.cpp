...
class GenerateProjectCategoriesPipelineStageTesting : public ::testing::Test {
protected:
...
TEST_F(GenerateProjectCategoriesPipelineStageTesting, matchingActivity) {
    ...
    generateProjectActivitiesPipelineStage(
            &pipeline,
            user_account_values
    );
    ...
    for(const auto& x : cursor_result) {
        ...
        bsoncxx::array::view activities_array = x[algorithm_pipeline_field_names::MONGO_PIPELINE_MATCHING_ACTIVITIES_FIELD].get_array().value;
...
TEST_F(GenerateProjectCategoriesPipelineStageTesting, noMatchingActivities) {
    ...
    for(const auto& x : cursor_result) {
        bsoncxx::array::view activities_array = x[algorithm_pipeline_field_names::MONGO_PIPELINE_MATCHING_ACTIVITIES_FIELD].get_array().value;
        int num_activities = std::distance(activities_array.begin(), activities_array.end());
