...
class RequestPhoneNumberTesting : public ::testing::Test {
protected:
...
TEST_F(RequestPhoneNumberTesting, invalidLoginInfo) {
...
TEST_F(RequestPhoneNumberTesting, successful) {
    runFunction();

    request_fields::InfoFieldResponse generated_response;

    generated_response.set_return_status(ReturnStatus::SUCCESS);
    generated_response.set_timestamp(-1);

    generated_response.set_return_string(user_account.phone_number);
