...
class SetEmailTesting : public testing::Test {
protected:
    ...
    void setupValidAdminRequest() {
        ...
        request.set_set_email("fake@email.com");
    }
...
TEST_F(SetEmailTesting, invalidLoginInfo) {
    setupValidUserRequest();
...
TEST_F(SetEmailTesting, noAdminPriveledge) {
    setupValidAdminRequest();
...
TEST_F(SetEmailTesting, invalidUserAccountOidPassedAsAdmin) {
    setupValidAdminRequest();
...
TEST_F(SetEmailTesting, invalidEmailAddress) {
    //NOTE: This is much more thoroughly check when checking isInvalidEmailAddress().
    const bool use_admin_info = randomizeClientOrAdmin();

    request.set_set_email("invalid_email");
...
TEST_F(SetEmailTesting, successful) {
    const bool use_admin_info = randomizeClientOrAdmin();

    user_account.email_address_requires_verification = false;
    ...
    user_account.email_address_requires_verification = true;
    user_account.email_address = request.set_email();
    user_account.email_timestamp = bsoncxx::types::b_date{std::chrono::milliseconds{response.timestamp()}};

    compareUserAccounts();

    user_account_statistics.email_addresses.emplace_back(
            user_account.email_address,
            user_account.email_timestamp
            );
