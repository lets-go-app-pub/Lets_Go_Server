...
class SetFirstNameTesting : public testing::Test {
protected:
    ...
    void setupValidUserRequest() {
        ...
        request.set_set_string("fake@email.com");
    }
...
TEST_F(SetFirstNameTesting, invalidLoginInfo) {
    setupValidUserRequest();
...
TEST_F(SetFirstNameTesting, noAdminPriveledge) {
    setupValidAdminRequest();
...
TEST_F(SetFirstNameTesting, invalidUserAccountOidPassedAsAdmin) {
    setupValidAdminRequest();
...
TEST_F(SetFirstNameTesting, invalidFirstName_tooShort) {
    const bool use_admin_info = randomizeClientOrAdmin();

    request.set_set_string("a");
...
TEST_F(SetFirstNameTesting, invalidFirstName_tooLong) {
    const bool use_admin_info = randomizeClientOrAdmin();

    request.set_set_string(std::string(server_parameter_restrictions::MAXIMUM_NUMBER_ALLOWED_BYTES_FIRST_NAME + 1, 'a'));
...
TEST_F(SetFirstNameTesting, invalidFirstName_invalidChar) {
    const bool use_admin_info = randomizeClientOrAdmin();

    request.set_set_string("Abigale8");
...
TEST_F(SetFirstNameTesting, successful) {
    const bool use_admin_info = randomizeClientOrAdmin();

    //make sure capitalization is correct
    request.set_set_string("rYaN");
    const std::string correct_name = "Ryan";
    ...
    user_account.first_name = correct_name;
    user_account.first_name_timestamp = bsoncxx::types::b_date{std::chrono::milliseconds{response.timestamp()}};
    user_account.last_time_displayed_info_updated = bsoncxx::types::b_date{std::chrono::milliseconds{response.timestamp()}};

    compareUserAccounts();

    user_account_statistics.names.emplace_back(
            user_account.first_name,
            user_account.first_name_timestamp
    );
