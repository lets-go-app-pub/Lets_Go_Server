...
class SetUserBioTesting : public testing::Test {
protected:
    ...
    const std::string default_bio = gen_random_alpha_numeric_string(rand() % 100 + 5);
    ...
    void setupValidAdminRequest() {
        ...
        request.set_set_string(default_bio);
    }
...
TEST_F(SetUserBioTesting, invalidLoginInfo) {
    setupValidUserRequest();
...
TEST_F(SetUserBioTesting, noAdminPriveledge) {
    setupValidAdminRequest();
...
TEST_F(SetUserBioTesting, invalidUserAccountOidPassedAsAdmin) {
    setupValidAdminRequest();
...
TEST_F(SetUserBioTesting, invalidFirstName_tooLong) {
...
TEST_F(SetUserBioTesting, successful) {
    const bool use_admin_info = randomizeClientOrAdmin();
    ...
    user_account.bio = request.set_string();
    user_account.bio_timestamp = bsoncxx::types::b_date{std::chrono::milliseconds{response.timestamp()}};
    ...
    user_account_statistics.bios.emplace_back(
            user_account.bio,
