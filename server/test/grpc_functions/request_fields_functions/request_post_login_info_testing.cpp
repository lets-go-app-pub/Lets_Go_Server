...
class RequestPostLoginInfoTesting : public ::testing::Test {
protected:
...
TEST_F(RequestPostLoginInfoTesting, invalidLoginInfo) {
...
TEST_F(RequestPostLoginInfoTesting, successful) {
    runFunction();

    request_fields::PostLoginInfoResponse generated_response;
    ...
    generated_response.mutable_post_login_info()->set_user_bio(user_account.bio);
    generated_response.mutable_post_login_info()->set_user_city(user_account.city);

    for(const auto& gender : user_account.genders_range) {
        generated_response.mutable_post_login_info()->add_gender_range(gender);
    }

    generated_response.mutable_post_login_info()->set_min_age(user_account.age_range.min);
    generated_response.mutable_post_login_info()->set_max_age(user_account.age_range.max);
    generated_response.mutable_post_login_info()->set_max_distance(user_account.max_distance);
