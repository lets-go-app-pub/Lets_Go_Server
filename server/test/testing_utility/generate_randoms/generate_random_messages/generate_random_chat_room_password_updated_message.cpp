...
grpc_chat_commands::UpdateChatRoomInfoRequest generateRandomChatRoomPasswordUpdatedMessage(
    ...
    std::string actual_new_chat_room_password;

    if(new_chat_room_password.empty()) {
        actual_new_chat_room_password = gen_random_alpha_numeric_string(rand() % 50);
    } else {
        actual_new_chat_room_password = new_chat_room_password;
    }

    update_chat_room_info_request.set_chat_room_id(chat_room_id);
    update_chat_room_info_request.set_type_of_info_to_update(grpc_chat_commands::UpdateChatRoomInfoRequest_ChatRoomTypeOfInfoToUpdate_UPDATE_CHAT_ROOM_PASSWORD);
    update_chat_room_info_request.set_new_chat_room_info(actual_new_chat_room_password);
